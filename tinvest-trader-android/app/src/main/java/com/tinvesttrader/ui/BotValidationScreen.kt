...
private fun ComparisonCard(result: BotBacktestResult) {
    ...
    Card(Modifier.fillMaxWidth().padding(top = 8.dp)) {
        Column(Modifier.padding(12.dp)) {
            ...
            Text(
                "Слева — текущие настройки: фильтры входа, подтягиваемый стоп-заявкой " +
                    "у брокера уровень от волатильности и досмотр пропущенных свечей. " +
                    "Справа — исходный бот: голое пересечение средних, только последняя " +
                    "свеча и мягкий стоп в 3%.",
                style = MaterialTheme.typography.labelSmall,
                ...
                when {
                    ...
                    betterExpectancy < -0.05 ->
                        "На этой бумаге правки среднюю сделку ухудшили на " +
                            "${percent(-betterExpectancy)}. Так бывает: фильтры отсеивают и " +
                            "часть удачных входов, а подтянутый стоп закрывает позицию на " +
                            "откате. Попробуйте другой таймфрейм или бумагу — и не включайте " +
                            "бота на той, где он проигрывает."
